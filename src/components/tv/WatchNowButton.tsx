...
export function WatchNowButton({ tmdbId, showName }: WatchNowButtonProps) {
    ...
    async function fetchSlug() {
      try {
        ...
        const response = await fetch(
          `https://trendimovies.com/api/series?tmdb_id=eq.${tmdbId}&select=slug`,
          { cache: "force-cache" }
        );
        ...
        // Fallback: generate slug from show name
        const generatedSlug = showName
          .toLowerCase()
          .replace(/[^a-z0-9\s-]/g, "")
          .replace(/\s+/g, "-")
          .replace(/-+/g, "-");
  ...
  const href = slug
    ? `https://trendimovies.com/series/${slug}`
    : `https://trendimovies.com/search?q=${encodeURIComponent(showName)}`;
